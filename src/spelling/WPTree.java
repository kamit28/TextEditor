...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) {
		...
		LinkedList<WPTreeNode> toExplore = new LinkedList<WPTreeNode>();
		...
		while (!toExplore.isEmpty() && !found) {
			ptr = toExplore.poll();
			if (ptr.getWord().equals(word2)) {
				found = true;
			} else {
				List<String> words = nw.distanceOne(ptr.getWord(), true);
				for (String word : words) {
					WPTreeNode child = ptr.addChild(word);
