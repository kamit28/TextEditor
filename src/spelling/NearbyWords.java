...
public class NearbyWords implements SpellingSuggest {
	 ...
	 * @return
	 */
	public void subsitution(String s, List<String> currentList,
			boolean wordsOnly) {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly) {
		...
		for (int j = (int) 'a'; j <= (int) 'z'; j++) {
			for (int i = 0; i <= s.length(); i++) {
				...
				sb.append((char) j).append(s.substring(i, s.length()));
				if (!currentList.contains(sb.toString())
						&& (!wordsOnly || dict.isWord(sb.toString()))
						&& !s.equals(sb.toString()))
					currentList.add(sb.toString());
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (!queue.isEmpty() && count < numSuggestions
			...
			newWords = distanceOne(str, true);
			for (String w : newWords) {
				if (!visited.contains(w) && count < numSuggestions) {
					queue.add(w);
					visited.add(w);
					retList.add(w);
					count++;
	...
	public static void main(String[] args) {
		...
		System.out.println("One away word Strings for for \"" + word
				+ "\" are:");
