...
public class TextProController {
	...
	private final static double RBOX_THRESHOLD = 520;
	...
	private void initialize() {
		...
		textBox = new AutoSpellingTextArea(launch.getAutoComplete(),
				launch.getSpellingSuggest(dic), dic);
		...
		Consumer<VBox> adjustSpacing = box -> {
			if (container.getHeight() < RBOX_THRESHOLD) {
				rightBox.setSpacing((container.getHeight() - CONTROL_HEIGHT)
						/ SPACE_DIV);
			} else {
