...
public class AutoSpellingTextArea extends StyledTextArea<Boolean> {
	...
	static {
		try {
			...
			mGetCharacterIndex = Class.forName(
					"org.fxmisc.richtext.skin.CharacterHit").getDeclaredMethod(
					"getCharacterIndex");
	...
	public AutoSpellingTextArea(spelling.AutoComplete ac,
			spelling.SpellingSuggest ss, spelling.Dictionary dic) {
		...
		this.setOnMouseClicked(e -> {
			if ((e.getButton() == MouseButton.SECONDARY) && spellingOn) {
				...
				OptionalInt opInt = (OptionalInt) invoke(mGetCharacterIndex,
						chHit);
	...
	private String getWordAtIndex(int pos) {
		...
		for (index = text.length() - 1; index >= 0
				&& !Character.isWhitespace(text.charAt(index)); index--)
			;
		...
		for (index = pos; index < this.getLength()
				&& !Character.isWhitespace(this.getText().charAt(index)); index++)
			;
	 ...
	 */
	private List<CustomMenuItem> createOptions(List<String> options,
			boolean[] flags) {
	...
	private void showSuggestions(String word, MouseEvent click) {
		...
		entriesPopup.show(getScene().getWindow(), click.getScreenX(),
				click.getScreenY());
	...
	private void showCompletions(String prefix) {
		...
		if (!prefix.equals("")) {
			...
			if (options.size() > 0) {
				...
				List<CustomMenuItem> menuOptions = createOptions(options,
						caseFlags);
